document.addEventListener("DOMContentLoaded", () => {
  ...
  const scoreComments = {
    6: {
      title: "🛡️ You're a Digital Guardian",
      comment: "You consistently made safe, informed choices and demonstrated strong digital awareness.",
      advice: "Keep sharing safe practices and stay alert to emerging online threats."
    },
    5: {
      title: "🛡️ You're a Digital Guardian",
      comment: "You made mostly safe decisions with good awareness of online risks.",
      advice: "Stay cautious — even small oversights can lead to harm."
    },
    4: {
      title: "⚠️ You're Risk-Aware but Vulnerable",
      comment: "You recognise risks but some choices could still expose you to online harm.",
      advice: "Pause, verify sources, and protect personal information."
    },
    3: {
      title: "⚠️ You're Risk-Aware but Vulnerable",
      comment: "You show partial awareness, but risky habits are still present.",
      advice: "Learn common scam patterns and practise setting boundaries."
    },
    2: {
      title: "🚨 There's High Risk Detected",
      comment: "Your decisions suggest a high likelihood of falling for online manipulation.",
      advice: "Avoid engaging with strangers and seek trusted advice."
    },
    1: {
      title: "🚨 There's High Risk Detected",
      comment: "You are highly vulnerable to online harms.",
      advice: "Review basic digital safety guidelines immediately."
    },
    0: {
      title: "🚨 There's High Risk Detected",
      comment: "Your digital safety awareness is very low.",
      advice: "It’s important to understand how scams and impersonation work."
    }
  };
...
function showFeedback(correct) {
  const fb = correct
    ? questions[current].feedback.correct
    : questions[current].feedback.wrong;
  ...
  modalContent.classList.remove("safe", "risky");

  modalContent.classList.add(correct ? "safe" : "risky");
  ...
  fBtn.onclick = () => {
    ...
    if (current < questions.length) {
      ...
      const result = scoreComments[score];

qText.innerHTML = `
  <strong class="${score >= 4 ? "safe" : "risky"}">
    ${result.title}
  </strong><br><br>
  <p>${result.comment}</p>
  <p><em>${result.advice}</em></p>
  <span class="final-caption">
  ...
  restartBtn.onclick = () => {
    window.location.href = "case01-1.html"; // first HTML page
  };
